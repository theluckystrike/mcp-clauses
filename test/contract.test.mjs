...
// Contract suite for clauses. Generated shape, mechanical assertions only.
//
// Asserts the invariants of servers/clauses/SPEC.md that a test can check without
...
function client(env = {}) {
  const home = mkdtempSync(join(tmpdir(), "mcp-contract-clauses-"));
...
test("corrupt store: the garbage is quarantined byte-for-byte and nothing is overwritten", async (t) => {
  ...
  const dir = join(c.home, "data", "mcp-servers", "clauses");
  mkdirSync(dir, { recursive: true });
  writeFileSync(join(dir, "data.json"), GARBAGE);

  await c.init();
  const r = await c.call("clause_add", {"title":"T","body":"B","category":"C"});
...
test.skip("cap: no row-capped write tool on this server", { skip: "clauses has no row cap on a written file; its free limits are counts of stored records, covered by the server's own suites" }, () => {});
