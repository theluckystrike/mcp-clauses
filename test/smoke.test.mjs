...
test.skip("free tier: the 11th own clause is refused, Pro takes it", async (t) => {
  const free = client();
  t.after(() => free.close());
  await init(free);
...
test.skip("import round trip through the server, markdown free and json in Pro", async (t) => {
  const key = execFileSync(process.execPath, [join(REPO, "scripts", "sign-license.mjs"), "clauses"], { encoding: "utf8" }).trim();
